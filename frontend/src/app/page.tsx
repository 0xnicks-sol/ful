...
import { Sword, Trophy, LayoutGrid, Shield, Zap } from "lucide-react"

// Token config — set NEXT_PUBLIC_TOKEN_MINT in frontend .env.local to override
const TOKEN_MINT   = process.env.NEXT_PUBLIC_TOKEN_MINT   || "6Jd9zLe8Bz8rQriciZCBw3ty9PjCbfaToLeF4Gxapump"
const TOKEN_NAME   = process.env.NEXT_PUBLIC_TOKEN_NAME   || "GLADIATOR"
const TOKEN_SYMBOL = process.env.NEXT_PUBLIC_TOKEN_SYMBOL || "GLAD"
    ...
    >
      ...
      <div>
        ...
        <p className="text-xs font-mono leading-relaxed" style={{ color: "#8a7a55" }}>
          The more tokens you purchase and hold, the greater your gladiator{"'"}s HP and battle strength. Larger holders deal more damage and take more hits before falling.
        </p>
      </div>
    </div>
  )
    ...
    <div className="relative min-h-screen flex flex-col" style={{ background: "#05050a" }}>
      ...
      <header className="relative z-50 sticky top-0">
        ...
        >
          ...
          <NavTab active={activeTab === "leaderboard"} onClick={() => setActiveTab("leaderboard")} icon={<Trophy className="w-3 h-3" />}>
            Leaderboard
          </NavTab>
              ...
              <div className="glass-card rounded-2xl p-4 flex flex-col gap-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs tracking-widest uppercase font-mono text-sand-dim">Top Winners</span>
                  </div>
                  <button
            ...
            <aside className="flex flex-col gap-3 xl:w-72 flex-shrink-0">
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-mono tracking-widest uppercase text-sand-dim">pump.fun Rules</span>
                </div>
                {[
                  ...
                  { dot: "#ff2e2e", text: "Last gladiator alive wins the round" },
                  { dot: "#ffd700", text: "Top 3 from 10+ rounds win 50% rewards" },
                ].map((item, i) => (
          ...
          <div className="p-4 max-w-6xl mx-auto w-full">
            ...
            <div className="flex flex-col lg:flex-row gap-4">
              ...
              <div className="lg:w-96 flex-shrink-0 flex flex-col gap-4">
                {(showLottery || isLotteryEligible) && grandWinners.length === 0 && (
                ...
                {grandWinners.length > 0 && (
                  <div className="glass-card rounded-2xl p-5 flex flex-col gap-4">
                    <div className="text-center">
                      ...
                      <h3 className="text-xl font-bold" style={{ fontFamily: "Georgia, serif", color: "#c9a227", textShadow: "0 0 20px rgba(201,162,39,0.4)" }}>
                        50% Rewards Split
                      </h3>
                    </div>
                    <p className="text-xs text-sand-dim font-mono leading-relaxed text-center">
                      Rewards distributed manually via pump.fun creator fees. Announcement posted on X.
                    </p>
                    <div className="flex flex-col gap-2">
                      {grandWinners.map((w, i) => (
                        <div key={w} className="rounded-xl p-3" style={{ background: "rgba(201,162,39,0.08)", border: "1px solid rgba(201,162,39,0.3)" }}>
                          <p className="text-xs font-mono font-bold mb-0.5" style={{ color: i === 0 ? "#ffd700" : i === 1 ? "#c0c0c0" : "#cd7f32" }}>
                    ...
                    <a href="https://x.com" target="_blank" rel="noopener noreferrer" className="text-xs font-mono text-center" style={{ color: "#c9a227" }}>
                      Follow on X for payout confirmation
                    </a>
                  </div>
                )}

                {/* Season stats */}
                <div className="glass-card rounded-2xl p-4 flex flex-col gap-3" style={{ border: "1px solid rgba(201,162,39,0.18)" }}>
                  <span className="text-xs font-mono tracking-widest uppercase text-sand-dim">Season Stats</span>
                  <div className="grid grid-cols-2 gap-3">
                    {[
                      { label: "Rounds Played",    value: totalRoundsCompleted,          color: "#c9a227" },
                      { label: "Total Fighters",   value: leaderboard.length,            color: "#4a9eff" },
                      { label: "Grand Winners",    value: grandWinners.length,           color: "#ffd700" },
                      { label: "Rounds to Lottery",value: Math.max(0, 10 - totalRoundsCompleted), color: "#22c55e" },
                    ].map((s) => (
      ...
      >
        <span className="text-xs font-mono text-sand-dim">GLADIATOR ARENA · pump.fun Battle Grounds · 24/7 LIVE</span>
        <div className="flex items-center gap-3">
          ...
          <a href="https://x.com" target="_blank" rel="noopener noreferrer" className="text-xs font-mono text-sand-dim hover:text-gold transition-colors">
            X / Twitter
          </a>
        </div>
      </footer>
